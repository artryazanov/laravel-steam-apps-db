...
class FetchSteamAppDetailsCommand extends Command
{
    ...
    public function handle(): void
    {
        ...
        $limit = (int) $argCount;
        $this->info("Starting to fetch Steam game details (count: {$limit})...");
        ...
        foreach ($appsToProcess as $index => $app) {
            ...
            try {
                ...
                $details = $this->fetchSteamAppDetails($app->appid);

                // Update the last_details_update field in the SteamApp model
                $app->update(['last_details_update' => Carbon::now()]);

                if (!$details) {
                    $this->warn("No details found for app {$app->name} (appid: {$app->appid})");
                    $failed++;
                ...
                try {
                    ...
                    DB::commit();
                    $success++;
                    $this->info("Successfully stored details for app {$app->name} (appid: {$app->appid})");
                } catch (\Exception $e) {
                    DB::rollBack();
                    $this->error("Failed to store details for app {$app->name} (appid: {$app->appid}): {$e->getMessage()}");
     ...
     *
     * Priority:
     * 1. Records with associated game and no loaded data
     * 2. Records without associated game and no loaded data
     * 3. Records with associated game and loaded data that haven't been updated for over a year
     *
     * @param int $limit
     * @return Collection
     */
     ...
     * @return array|null
     */
    private function fetchSteamAppDetails(int $appid): ?array
    {
        ...
        if (!$response->successful()) {
            $this->error("Failed to fetch details for appid {$appid}: " . $response->status());
            return null;
        }
        ...
        if (!isset($data[$appid]['success']) || !$data[$appid]['success']) {
            $this->warn("No success response for appid {$appid}");
            return null;
        }

        if (!isset($data[$appid]['data'])) {
            $this->warn("No data found for appid {$appid}");
            return null;
        }
